...
def get_id(url):
    print(re.match(r'/posts/(.+)/', url).group(1))
...
class UrlView:
    ...
    @staticmethod
    def do_GET_json(url=None, vars=None):
    ...
    @staticmethod
    def do_POST(url=None, vars=None):
    ...
    @staticmethod
    def do_DELETE(url=None, vars=None):
        if os.path.isfile(FILE_NAME):
            with open(FILE_NAME, 'r+') as f:
                if check_not_empty_file():
                    posts_data = load_data_file(f)
